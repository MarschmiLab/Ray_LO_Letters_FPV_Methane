---
title: "Water Column and Sediment Methanogens & Methanotrophs in FSP ponds"
author: "sophiaaredas"
...
knitr::opts_chunk$set(eval = TRUE, 
                      ...
                      error = TRUE,
                      fig.path="../figures/Nick_Analysis_GHGs/",  
                      fig.align = "center") 
```
...
load("data/06A_Water_Compositional_Analysis_SA/full_abs_physeq.RData")

# sediment scaled physeq
load("data/06B_Sediment_Compositional_Analysis_SA/scaled_sed_physeq.RData")

# physeq with water (unincorporated cell counts) + sediment samples = 188 samples total
load("data/04_Phylogenetics_SA/new_archaea_rooted_physeq.RData")


# load in metadata
load("data/06A_Water_Compositional_Analysis_SA/metadata.RData")
```
...
save(water_ch4_physeq, file = "data/FSP_2024_Nick/water_ch4_physeq.RData")
...
save(water_physeq_ch4_order, file = "data/FSP_2024_Nick/water_physeq_ch4_order.RData")

# write out to excel file for plotting main figure
write.csv(methano_water_df, "data/FSP_2024_Nick/watercol_microbial_communities.csv")
```
We created:
...
# subset samples that were switched
...
# save .RData
save(methano_sed_df, file = "data/FSP_2024_Nick/methano_sed_df.RData")


# write out to excel file 
write.csv(methano_sed_df, "data/FSP_2024_Nick/sed_microbial_communities.csv")
...
# save phyloseq object
save(scaled_sed_ch4_physeq, file = "data/FSP_2024_Nick/scaled_sed_ch4_physeq.RData")
...
# save .RData
save(methanogen_sed_df, file = "data/FSP_2024_Nick/methanogen_sed_df.RData")
...
# save phyloseq object
save(scaled_methanogen_sed_physeq, file = "data/FSP_2024_Nick/scaled_methanogen_sed_physeq.RData")
```
We created:
...
# save .RData
save(methanotroph_sed_df, file = "data/FSP_2024_Nick/methanotroph_sed_df.RData")
...
# save phyloseq object
save(scaled_methanotroph_sed_physeq, file = "data/FSP_2024_Nick/scaled_methanotroph_sed_physeq.RData")
```
We created:
...
```
Based on our data with the qq plots we see that the points more or less fit the line. but when we investigate further with the density histograms, it does not look like a clear unimodal distribution distribution but lets check this out further with a Shapiro-Wilk test. When we run our Shapiro-Wilk test to also test for a normal distribution if p > 0.05 the data is likely normal but if p < 0.05 then the data is not normal.
...
# plot finals
surf_depths <- 
  ggarrange(methanogen_surfwater24, box_gen_surf, methanotrophs_surfwater24, box_troph_surf,
  nrow = 2, 
  ncol = 2,
  align = "hv",
  widths = c(2, 1))
...
ggsave(
  "analysis/FSP_Nick_Analysis/figures/surf_depths.png",
  surf_depths,
  width = 7,            
  height = 5,         
  units = "in",
...
bot_depths <- ggarrange(methanogen_bot_water24, box_gen_bot, methanotroph_bot_water24, box_troph_bot,
  nrow = 2, 
  ncol = 2,
  align = "hv",
  widths = c(2, 1))
...
ggsave(
  "analysis/FSP_Nick_Analysis/figures/bot_depths.png",
  bot_depths,
  width = 7,            
  height = 5,         
  units = "in",
...
sed_depths <-
  ggarrange(methano_sed_24, box_gen_sed, methanotroph_sed_24, box_troph_sed,
  nrow = 2, 
  ncol = 2,
  align = "hv",
  widths = c(2, 1))
...
ggsave(
  "analysis/FSP_Nick_Analysis/figures/sed_depths.png",
  sed_depths,
  width = 7,            
  height = 5,         
  units = "in",
...
# Display
sed_depths_leg  # so here we will do a little cropping in Illustrator to make it look right. 

ggsave(
  "analysis/FSP_Nick_Analysis/figures/sed_depths_leg.png",
  sed_depths_leg,
  width = 7,            
  height = 5,         
  units = "in",
...
```
now we have created our main text figure. the only thing is that i dont have the p values in th eifgure because nick has his italicized and sized a certain way and i want to make sure our plot aesthetics match.


# Supplemental Figures
Here are the supplemental figures (and bonus figures) for the manuscript. 

## Figure S1 - Water and Sediment Bray-Curtis PCoA
...
# ggsave(s3_ch4_pcoa, width = 8, height = 7, units = "in",
#         filename = "analysis/FSP_Nick_Analysis/figures/s1a_water_pcoa.png")
```
...
# ggsave(sed_pond_solar_pcoa, width = 8, height = 7, units = "in",
#         filename = "analysis/FSP_Nick_Analysis/figures/sed_pond_solar_pcoa.png")
```
...
ggsave(fig_s1, width = 12.4, height = 6, dpi = 300,
        filename = "analysis/FSP_Nick_Analysis/figures/fig_s1.png")
```
...
water_bray <- phyloseq::distance(water_ch4_physeq, method = "bray", binary = FALSE)
...
adonis2(water_bray ~ solar_progress, data = metadata, by = "terms")

# Now testing to see if centroids of depth_class are different: not significant p = 0.19
adonis2(water_bray ~ Depth_Class, data = metadata, by = "terms")

# Does pond matter? significant p = 0.001 ***
adonis2(water_bray ~ Pond, data = metadata, by = "terms")

# Does date matter? significant p = 0.001 ***
adonis2(water_bray ~ as.factor(JDate), data = metadata, by = "terms")

#2. Test the terms together
# Now lets see the effect of each pond by date_collected and solar progress
adonis2(water_bray ~ solar_progress * Pond * JDate, data = metadata, by = "terms")
```
...
# calculate Bray-Curtis PERMANOVA using phyloseq distance
sed_bray <- phyloseq::distance(scaled_sed_physeq, method = "bray", binary = FALSE)

# pull out metadata 
metadata <- scaled_sed_physeq %>%
  sample_data() %>%
  data.frame()
...
#1. Test the individual terms for significance
# Testing if the centroids of solar progress are different: significant p = 0.001 ***
adonis2(sed_bray ~ solar_progress, data = metadata, by = "terms")

# Does pond matter? significant p = 0.001 ***
adonis2(sed_bray ~ Pond, data = metadata, by = "terms")

# Does date matter? significant p = 0.001 ***
adonis2(sed_bray ~ as.factor(JDate), data = metadata, by = "terms")

#2. Test the terms together
# Now lets see the effect of each pond by date_collected and solar progress
adonis2(sed_bray ~ solar_progress * Pond * JDate, data = metadata, by = "terms")
```
...
## Bray-Curtis
betadispr_water_pond <- betadisper(water_bray, metadata$Pond)
betadispr_water_solar <- betadisper(water_bray, metadata$solar_progress)
betadispr_water_depth <- betadisper(water_bray, metadata$Depth_Class)
betadispr_water_JDate <- betadisper(water_bray, metadata$JDate)
...
permutest(betadispr_water_pond) # not significant p = 0.269 

permutest(betadispr_water_solar) # significant p = 0.009 **

permutest(betadispr_water_depth) # not significant p = 0.406 

permutest(betadispr_water_JDate) # significant p = 0.007 **
```
...
## Bray-Curtis
betadispr_sed_pond <- betadisper(sed_bray, metadata$Pond)
betadispr_sed_solar <- betadisper(sed_bray, metadata$solar_progress)
betadispr_sed_JDate <- betadisper(sed_bray, metadata$JDate)

# permutest() performs a non-parametric permutation test, which is robust and valid for the kind of data used in beta diversity analysis (e.g., dissimilarity matrices).
permutest(betadispr_sed_pond) # not significant p = 0.756

permutest(betadispr_sed_solar) # not significant p = 0.203

permutest(betadispr_sed_JDate) # not significant p = 0.196
```
...
ggsave(fig_s2, width = 12.4, height = 6, dpi = 300,
        filename = "analysis/FSP_Nick_Analysis/figures/fig_s2.png")
```
Sediment samples are still distinct from other and separate along first axis
...
# save(water_ch4_asv_output, file = "data/FSP_2024_Nick/water_ch4_asv_output.RData")

load("data/FSP_2024_Nick/water_ch4_asv_output.RData")
...
# save(sed_ch4_asv_output, file = "data/FSP_2024_Nick/sed_ch4_asv_output.RData")

load("data/FSP_2024_Nick/sed_ch4_asv_output.RData")
...
methano_sed_asv_df <- methano_sed_phy 
...
png("analysis/FSP_Nick_Analysis/figures/s3_community_comp.png", width = 3900, height = 4000, res = 600)
