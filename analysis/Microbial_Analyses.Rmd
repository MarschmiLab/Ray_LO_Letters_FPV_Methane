...
#save(water_physeq_24, file = "data/01_phyloseq/water_ch4_physeq_24.RData")
```
...
scaled_sed_physeq_24 <- scaled_sed_physeq_24 %>% 
  speedyseq::psmelt() # melt into dataframe 
...
left off here: filtering out for methanogens and methanotrophs specifically!
...
water_ch4_cyclers_asv_df <- asv_df %>% 
  dplyr::filter(str_detect(function_group, "^meth")) # filtering for methanogens / methanotroph. will get some others but we will filter those out


# join sulfur reducers with water_physeq_asv
water_ _joined <- water_ch4_cyclers_asv_df %>%
  dplyr::inner_join(sulfate_reducer_asv_df, 
                    by = c("ASV" = "asv_id"))

# 661 total asvs

# check out unqiue taxanomic levels
sulfate_reducer_joined$Phylum %>% unique()
sulfate_reducer_joined$Order %>% unique()


# lets do a rough plot of who is here 
sulfate_df <- water_physeq_asv_df %>%
  dplyr::filter(
    solar_progress != "In Progress",
    ASV %in% sulfate_reducer_joined$ASV) %>% 
  dplyr::group_by(
    Year, solar_progress, Depth_Class, Phylum,
    Genus
  ) %>%
  dplyr::summarize(
  ...
  dplyr::mutate(
    metabolic_group = "Sulfate Reducers"
  )

#sulfate_df$Phylum %>% unique()

# sulfate_df <- sulfate_df %>%
#   dplyr::filter(!is.na(Genus)) 

# violin
sulf_plot <- sulfate_df %>%
  #dplyr::group_by(Genus) %>%
 # dplyr::filter(all(tapply(total_cells_ml, solar_progress, sum) > 0)) %>%  # keep genera where solar_progress groups cells have more than 0
  #dplyr::ungroup() %>%
  dplyr::filter(Phylum == "Desulfobacterota_G_459546") %>% 
  ggplot(aes(x = solar_progress, y = total_cells_ml, color = solar_progress)) +
  geom_violin(aes(fill = solar_progress, color = solar_progress), trim = TRUE, alpha = 0.5, width = 1) +
  geom_boxplot(outlier.shape = NA, width = 0.1, color = "black",position = position_dodge(0.6), alpha = 0) +
  # geom_point(size = 2, alpha = 0.8, shape = 16, color = "black",
  #   position = position_jitterdodge(jitter.width = 0.25, dodge.width = 0.6)) +
  geom_point(
  aes(fill = solar_progress),   # inside color
  ...
  # geom_violin(aes(fill = solar_progress), trim = FALSE, alpha = 0.5, width = 1) +
  # geom_boxplot(width = .1, outlier.shape = NA, alpha = 0, color = "black", position = position_dodge(0.6)) +
  labs(color = "Treatment", x = NULL, y = "Cells per mL (log10)") + 
  #ggh4x::facet_nested_wrap(Year ~ polished_tax, scales = "free_y", nrow = 2, ncol = 4) +
  #ggh4x::facet_grid2(vars(Year), vars(polished_tax), render_empty = TRUE) +
  ggh4x::facet_grid2(metabolic_group ~Year, scales = "free_y") +
  scale_color_manual(values = solar_colorsv2) +
  scale_fill_manual(values = solar_colorsv2) +
  scale_y_log10(
    labels = scales::label_number(scale_cut = scales::cut_short_scale()),
    name = expression("log10[Absolute Abundance]")
  ) +
 # scale_shape_manual(values = pond_shapes) +
  # scale_y_continuous(labels = scales::label_number(scale_cut = scales::cut_short_scale()),
  #                    name = expression("Cells ("*10^6*"/mL)")) +
  # scale_y_continuous(
  # trans = scales::pseudo_log_trans(base = 10, sigma = 30000),
  # labels = scales::label_number(scale_cut = scales::cut_short_scale()),
  # name = expression("Cells per mL")) +
  theme_fpv +
  # guides(
...
``` {plotfaprov2results}
# load in function table. should have function as rows, columns as samples, and each sample has abundance for that given function 
ch4_water_faprov2 <- read_tsv("data/02A_Water_FAPROTAXv2/ch4_water_function_table_16S.tsv.tsv")

# get output 
ch4_water_fx <- ch4_water_faprov2 %>%
  dplyr::rename(metabolic_function = `function`) %>% 
  pivot_longer(matches("*SR*|SA*"), names_to = "DNA_ID", values_to = "Count") %>% 
  group_by(metabolic_function, DNA_ID) %>%
  dplyr::summarize(med_count = median(Count)) %>% 
  ungroup() %>%
  group_by(metabolic_function) %>% # Group by functional annotation
  filter(any(med_count!=0)) %>% # Get rid of any groups that are all zero
  ungroup() %>% 
  left_join(., metadata, by = "DNA_ID") %>% # join metadata
  group_by(solar_progress, metabolic_function) %>%
  dplyr::summarize(mean_count = mean(med_count),
            sum_med = sum(med_count)) %>% # For each functional group, find the mean count in that group and total number of observations
  ungroup() %>%
  dplyr::group_by(metabolic_function) %>% 
  dplyr::mutate(rel_abund = mean_count / sum(mean_count),
         sum_count = sum(sum_med),
         sum_count = round(sum_count)) %>% 
         # metabolic_function = paste0(metabolic_function, " (", sum_count, ")"))
         # metabolic_function = str_replace_all(metabolic_function, "_", " "),
         # metabolic_function = str_to_title(metabolic_function)) %>% # For each functional group, calculate the relative abundance of counts in that Comp_Group
  separate(metabolic_function, into = c("metabolic_function", "extra"), sep = "\\.", remove = TRUE) %>%
  dplyr::select(-extra) %>% 
  dplyr::filter(sum_count > 100) # Only include groups that we observed at least 100 counts for - might include later

wide_fapro <- ch4_water_fx %>%
  dplyr::select(-mean_count, -sum_count, -sum_med) %>%
  pivot_wider(names_from = metabolic_function, values_from = rel_abund, values_fill = 0) # Convert to wide (in preparation for matrix)

sum_fapro_mat <- as.matrix(wide_fapro[-1]) # Convert to matrix (drop grouping column)

rownames(sum_fapro_mat) <- wide_fapro$solar_progress # Add grouping column back as rownames

# Calculate distance, cluster, and find the order of each comp group
comp_group_order <- dist(sum_fapro_mat) %>%
  hclust() %>%
  as.dendrogram() %>%
  order.dendrogram()

# Calculate distance, cluster, and find the order for each functional group
func_group_order <- dist(t(sum_fapro_mat)) %>%
  hclust() %>%
  as.dendrogram() %>%
  order.dendrogram()


heatmap_plot <- ch4_water_fx %>%
  arrange(desc(rel_abund)) %>% 
  mutate(solar_progress = factor(solar_progress, levels = rownames(sum_fapro_mat)[comp_group_order]),
         group = factor(metabolic_function, levels = colnames(sum_fapro_mat)[func_group_order])) %>% # Reorder factor levels based on dendrogram
  ggplot(aes(x = solar_progress, y = metabolic_function, fill = round(rel_abund, digits = 2))) + 
  geom_tile(color = "black") +
  scale_fill_gradient2(low = "dodgerblue3", mid = "white", high = "firebrick",
                       na.value = "white",
                       name = NULL, midpoint = 0.5) +
  geom_text(aes(label = round(rel_abund, digits = 2)), color = "black", size = 3) +
  labs(x = NULL, y = NULL, title = "Relative Abundance of Detection within metabolic group") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = "none",
        panel.grid = element_blank(), text = element_text(size = 7))

# Plot total observations as single-column heatmap
n_plot <- ch4_water_fx %>%
  mutate(solar_progress = factor(solar_progress, levels = rownames(sum_fapro_mat)[comp_group_order]),
         group = factor(metabolic_function, levels = colnames(sum_fapro_mat)[func_group_order])) %>%
  ggplot(aes(x="1", y = metabolic_function, fill = log10(sum_count))) + 
  geom_tile() + 
  theme_void() + 
  labs(fill = "Log10 Total\nCounts") 

# Arrange together
faprotaxv2_water_24 <- ggarrange(plotlist =  list(heatmap_plot, NULL, n_plot), widths = c(2,-.5,1), align = "hv", nrow = 1)
faprotax_23_24

# plot relevant functions
sig_fapro <- ch4_water_fx %>% 
  dplyr::filter(rel_abund < 0.45 | rel_abund > 0.55) %>%
  dplyr::filter(
    metabolic_function == "anaerobic_chemoheterotrophy"| 
    metabolic_function == "aerobic_chemoheterotrophy"| 
    metabolic_function == "aerobic_anoxygenic_phototrophy" | 
    metabolic_function == "anoxygenic_photoautotrophy" | 
    metabolic_function == "anoxygenic_photoautotrophy_S_oxidizing" | 
    metabolic_function == "aromatic_compound_degradation" |
    metabolic_function == "anoxygenic_photoautotrophy_Fe_oxidizing" |
    metabolic_function == "anoxygenic_photoautotrophy_H2_oxidizing" |
    metabolic_function == "dark_sulfide_oxidation" | 
    metabolic_function == "dark_sulfite_oxidation" | 
    metabolic_function == "iron_respiration" |
    metabolic_function == "iron_oxidation" |
    metabolic_function == "manganese_respiration" | 
    metabolic_function == "methanotrophy" |
    metabolic_function == "methylotrophy" | 
    metabolic_function == "photoheterotrophy" | 
    metabolic_function == "respiration_of_sulfur_compounds" |
    metabolic_function == "sulfur_reduction" | 
    metabolic_function == "sulfur_respiration"|
    metabolic_function == "oxygenic_photoautotrophy" | 
    metabolic_function == "nonphotosynthetic_cyanobacteria" |
    metabolic_function == "obligate_anaerobes" | 
    metabolic_function == "nitrogen_fixation"| 
    metabolic_function == "dissimilatory_arsenate_reduction" | 
    metabolic_function == "reductive_acetogenesis")
sig_fapro

sig_wide_fapro <- sig_fapro %>%
  dplyr::select(-mean_count, -sum_count, -sum_med) %>%
  pivot_wider(names_from = metabolic_function, values_from = rel_abund, values_fill = 0) # Convert to wide (in preparation for matrix)

sig_sum_fapro_mat <- as.matrix(sig_wide_fapro[-1]) # Convert to matrix (drop grouping column)

rownames(sig_sum_fapro_mat) <- sig_wide_fapro$solar_progress # Add grouping column back as rownames

# Calculate distance, cluster, and find the order of each comp group
sig_comp_group_order <- dist(sig_sum_fapro_mat) %>%
  hclust() %>%
  as.dendrogram() %>%
  order.dendrogram()

# Calculate distance, cluster, and find the order for each functional group
sig_func_group_order <- dist(t(sig_sum_fapro_mat)) %>%
  hclust() %>%
  as.dendrogram() %>%
  order.dendrogram()


sig_heatmap_plot <- sig_fapro %>%
  mutate(solar_progress = factor(solar_progress, levels = rownames(sig_sum_fapro_mat)[sig_comp_group_order]),
         group = factor(metabolic_function, levels = colnames(sig_sum_fapro_mat)[sig_func_group_order])) %>% # Reorder factor levels based on dendrogram
  ggplot(aes(x = solar_progress, y = metabolic_function, fill = round(rel_abund, digits = 2))) + 
  geom_tile(color = "black") +
  scale_fill_gradient2(low = "dodgerblue3", mid = "white", high = "firebrick",
                       na.value = "white",
  ...
  theme(plot.title = element_text(hjust = 0.9),
        legend.position = "none",
        panel.grid = element_blank(), text = element_text(size = 9))

# Plot total observations as single-column heatmap
sig_n_plot <- sig_fapro %>%
  mutate(solar_progress = factor(solar_progress, levels = rownames(sig_sum_fapro_mat)[sig_comp_group_order]),
         group = factor(metabolic_function, levels = colnames(sig_sum_fapro_mat)[sig_func_group_order])) %>%
  ggplot(aes(x="1", y = metabolic_function, fill = log10(sum_count))) + 
  geom_tile() + 
  theme_void() + 
  labs(fill = "Log10 Total\nCounts") 
...
# Arrange together
pmm <- ggarrange(plotlist =  list(sig_heatmap_plot, NULL, sig_n_plot), widths = c(4,-1,1.4), align = "hv", nrow = 1)
pmm

# ggsave(p1, width = 8, height = 7, units = "in",
#         filename = "analysis/figures/10A_Water_Functional_Annontation_SA/p1.png")


# Binomial
hsp_bn <- read_tsv("data/10A_Water_Functional_Annontation_SA/faprotax2/function_table_16sfastav5_bionomial.tsv")

# different output than before
plot_new_f2_fx1 <- hsp_bn %>%
  dplyr::rename(metabolic_function = `function`) %>% 
  pivot_longer(matches("*SR*|SA*"), names_to = "DNA_ID", values_to = "Count") %>% 
  group_by(metabolic_function, DNA_ID) %>%
  dplyr::summarize(med_count = median(Count)) %>% # Combine rarefied replicates
  ungroup() %>%
  group_by(metabolic_function) %>% # Group by functional annotation
  filter(any(med_count!=0)) %>% # Get rid of any groups that are all zero
  ungroup() %>% 
  left_join(., metadata, by = "DNA_ID") %>% # join metadata
  dplyr::filter(solar_progress != "Pre-FPV") %>% # filter out in progress
  group_by(solar_progress, metabolic_function) %>%
  dplyr::summarize(mean_count = mean(med_count),
            sum_med = sum(med_count)) %>% # For each functional group, find the mean count in that group and total number of observations
  ungroup() %>%
...
# Arrange together
faprotax_23_24 <- ggarrange(plotlist =  list(heatmap_plot, NULL, n_plot), widths = c(2,-.5,1), align = "hv", nrow = 1)
faprotax_23_24

# plot relevant functions
sig_fapro <- plot_new_f2_fx1 %>% 
  dplyr::filter(rel_abund < 0.45 | rel_abund > 0.55) %>%
  dplyr::filter(
    metabolic_function == "anaerobic_chemoheterotrophy"| 
    metabolic_function == "aerobic_chemoheterotrophy"| 
...
# Arrange together
pbn <- ggarrange(plotlist =  list(sig_heatmap_plot, NULL, sig_n_plot), widths = c(4,-1,1.4), align = "hv", nrow = 1)
pbn

# Nearest Neighbot
hsp_nn <- read_tsv("data/10A_Water_Functional_Annontation_SA/faprotax2/function_table_16sfastav6_nn.tsv")

# different output than before
plot_new_f2_fx1 <- hsp_nn %>%
  dplyr::rename(metabolic_function = `function`) %>% 
  pivot_longer(matches("*SR*|SA*"), names_to = "DNA_ID", values_to = "Count") %>% 
  group_by(metabolic_function, DNA_ID) %>%
  dplyr::summarize(med_count = median(Count)) %>% # Combine rarefied replicates
...
# Arrange together
pnn <- ggarrange(plotlist =  list(sig_heatmap_plot, NULL, sig_n_plot), widths = c(4,-1,1.4), align = "hv", nrow = 1)
pnn


# No hsp flag
no_hsp <- read_tsv("data/10A_Water_Functional_Annontation_SA/faprotax2/function_table_16sfastav4.tsv")

# different output than before
plot_new_f2_fx1 <- no_hsp %>%
  dplyr::rename(metabolic_function = `function`) %>% 
  pivot_longer(matches("*SR*|SA*"), names_to = "DNA_ID", values_to = "Count") %>% 
  group_by(metabolic_function, DNA_ID) %>%
  dplyr::summarize(med_count = median(Count)) %>% # Combine rarefied replicates
...
# Arrange together
pNoHSP <- ggarrange(plotlist =  list(sig_heatmap_plot, NULL, sig_n_plot), widths = c(4,-1,1.4), align = "hv", nrow = 1)
pNoHSP

# now lets evaluate plots tgt
fapro_outputs <- 
  ggarrange(pmm, pbn, pnn, pNoHSP,
  nrow = 2, 
  ncol = 2,
  align = "hv",
  labels = c("Markov.", "Binomial.", "NN.", "None."),
...
```{r}
# First, you'll need to calculate relative abundances and get metadata
# Assuming you have a metadata file that maps DNA_ID to groups

# Read metadata (adjust path as needed)
metadata <- read_tsv("path/to/your/metadata.tsv")

count_fapro <- faprotax2 %>%
  pivot_longer(-`function`, names_to = "Sample", values_to = "Abundance" * 100) %>%
  mutate(count = ifelse(Abundance > 0, 1, 0))  
# Join with your functional data and calculate relative abundances
heatmap_data <- plot_new_f2_fx %>%
  left_join(metadata, by = "DNA_ID") %>%
  group_by(DNA_ID) %>%
  mutate(rel_abund = Count / sum(Count)) %>%
  ungroup()

# If you want to reorder based on clustering like in your example:
# First create a matrix for clustering
sum_fapro_mat <- heatmap_data %>%
  select(metabolic_function, DNA_ID, rel_abund) %>%
  pivot_wider(names_from = DNA_ID, values_from = rel_abund, values_fill = 0) %>%
  column_to_rownames("metabolic_function") %>%
  as.matrix()

# Perform clustering
comp_group_order <- hclust(dist(sum_fapro_mat))$order  # Cluster functions
func_group_order <- hclust(dist(t(sum_fapro_mat)))$order  # Cluster samples

# Create the heatmap
heatmap_plot <- heatmap_data %>%
  mutate(
    metabolic_function = factor(metabolic_function, 
                               levels = rownames(sum_fapro_mat)[comp_group_order]),
    DNA_ID = factor(DNA_ID, 
                   levels = colnames(sum_fapro_mat)[func_group_order])
  ) %>%
  ggplot(aes(x = DNA_ID, y = metabolic_function, fill = rel_abund)) + 
  geom_tile(color = "black") +
  scale_fill_gradient2(
    low = "dodgerblue3", mid = "white", high = "firebrick",
    na.value = "white",
    name = "Relative\nAbundance", midpoint = 0.5
  ) +
  geom_text(aes(label = round(rel_abund, digits = 2)), color = "black", size = 3) +
  labs(x = NULL, y = NULL, title = "Relative Abundance of Metabolic Functions") +
  theme_minimal() +
  theme(
...
# Combine plots if desired
library(patchwork)
combined_plot <- n_plot + heatmap_plot + plot_layout(widths = c(1, 10))
combined_plot
```
...
# 1. Ensure function table is in long format
plot_new_f2_fx1_long <- faprotax2 %>%
  dplyr::rename(metabolic_function = `function`) %>%
  pivot_longer(-metabolic_function, names_to = "DNA_ID", values_to = "Rel_abund") %>%
  mutate(Rel_abund = as.numeric(Rel_abund))

# 2. Calculate total ASV counts per sample
sample_totals <- classic_otu_table_ex3 %>%
  pivot_longer(-OTU, names_to = "DNA_ID", values_to = "Abundance") %>%
  group_by(DNA_ID) %>%
  summarise(Total_counts = sum(Abundance), .groups = "drop")

# 3. Approximate absolute abundance per function
plot_new_f2_fx1_abs <- plot_new_f2_fx1_long %>%
  left_join(sample_totals, by = "DNA_ID") %>%
  mutate(Abs_abund = Rel_abund * Total_counts) %>% 
  left_join(metadata, by = "DNA_ID")

# 4. Optional: log-transform for heatmap scaling
plot_new_f2_fx1_abs <- plot_new_f2_fx1_abs %>%
  mutate(Log10_Abs = log10(Abs_abund + 1))

# 5. Heatmap of approximate absolute abundances
ggplot(plot_new_f2_fx1_abs, aes(x = solar_progress, y = metabolic_function, fill = Log10_Abs)) +
  geom_tile(color = "black") +
  scale_fill_gradient2(low = "dodgerblue3", mid = "white", high = "firebrick",
                       midpoint = median(plot_new_f2_fx1_abs$Log10_Abs),
                       name = "Log10 Approx. Abundance") +
  facet_wrap(~solar_progress, scales = "free_x") +
  theme_minimal() +
...
``` {r water-phyloseq1234}
# melt physeq into data frame for just methane cyclers at Order level

# pull unique methane cycler taxonomic information , this will be updated shortly bc wrong rn
...
# filter phyloseq, trying to preserve abundance values
water_ch4_physeq_24 <- subset_taxa(
  water_physeq_24,
  Order %in% c(methanogens, methanotrophs)
)

# to mutate if methanogen or methanotroph convert to df
tax_df_2024 <- as.data.frame(tax_table(water_ch4_physeq_24))

# add designation
tax_df_2024 <- tax_df_2024 %>%
  mutate(
    Methanotroph_Methanogen = case_when(
      Order %in% methanogens ~ "Methanogen",
      Order %in% methanotrophs ~ "Methanotroph",
...
# convert back to matrix and put back into phyloseq object format
tax_table(water_ch4_physeq_24) <- tax_table(as.matrix(tax_df_2024))

# intuition check to see if we made column in tax table
head(as.data.frame(tax_table(water_ch4_physeq_24))) # looks good!

# old filtering that i think was messing up abudnances
...
# summarize methane cyclers by order and sample metadata
water_physeq_ch4_order <- water_ch4_physeq_24 %>%
  ...
  group_by(Pond, solar_progress, Depth_Class, Methanotroph_Methanogen, JDate, Phylum, Class, Order, Family, Genus, ASV) %>% 
  summarise(
    total_cells_ml = sum(Abundance, na.rm = TRUE),
    .groups = "drop"
  )

# now make dataframe for plotting
methano_water_df <- water_physeq_ch4_order %>%
  mutate(
    Depth_Class = recode(Depth_Class,
      "S" = "Surface Water",
      "B" = "Bottom Water"
    ),
    Depth_Class = factor(Depth_Class, levels = c("Surface Water", "Bottom Water")),
    solar_progress = recode(solar_progress,
      "Solar" = "FPV",
      "No Solar" = "Open"
    ),
  ...
  select(Pond, JDate, Depth_Class, solar_progress, Methanotroph_Methanogen, total_cells_ml, solar_methano, ASV)
...
# Identify putative CH4 cyclers
Here we will take our water and sediment physeq objects and run the latest version of FAPROTAXV2. This takes either taxonomic names, trained on SILVA database but does recognize some GreenGenes but with the GreenGenes2 update, i can confirm it predicts marginally less due to the renaming. Thus we will utilize the 16S rRNA seuqences where it places our sequences against a phylogenetic reference tree to determine putative metabolic functions. It does use different hidden state prediction algorithms but I will be sticking with the default (binomial) for now since the optimized version is still under development. 
